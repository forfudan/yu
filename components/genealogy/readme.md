...
- **截圖下載**：點擊 📷 按鈕可將當前可見的繫絡圖保存為 PNG 圖片
...
- **connectionRenderer.ts**：連接線樣式和路徑生成
- **exportService.ts**：將繫絡圖導出為 PNG 圖片
...
- **保持狀態**：截圖會保留當前的所有篩選、關注狀態和佈局
- **保持主題**：使用當前的主題顏色（亮色或暗色模式）
- **高清晰度**：使用 2x 縮放比例
- **自動優化**：
  - 隱藏工具欄和浮動提示
  - 添加標題欄和水印
  - 根據當前主題自動調整顏色
- **兼容性**：基於 `html2canvas-pro` 庫，支持現代瀏覽器
- **文件命名**：自動生成帶日期的文件名，如 `漢字字形輸入法繫絡圖20260110.png`

### 注意事項

- 導出過程中請勿滾動或切換標籤頁
- 首次導出可能需要幾秒鐘加載
- 截圖會包含當前可見的所有內容，建議調整好視圖後再導出
