...
拆字，就是把漢字按照一定的規則，拆成唯一的、有序的字根組合。這個過程如同拼圖一般，大多數時候，是非常直觀而自然的。這種將字拆成直觀的、分離的部件的方法，足以應付大部分漢字的拆分。
...
「<!-- verbatim -->國<!-- verbatim -->」如果完全符合筆順，應該拆成「冂一口一𠄌丿丶一」，是八個字根。但拆成「囗戈口一」，只用四個字根就夠了。因爲「少」優先級高於「順」，所以拆成「囗戈口一」。
...
結構完整，指的是存在**半包圍**形態的字根和存在**全包圍**形態的字根不拆散。包括：
...
「聿」可以拆成「彐」也可以拆成「肀二」，都是兩根，但拆成「彐」時，全部字根一體寫就。
...
「妻」可以拆成「十彐女」也可以拆成「コ女」，都是三根。拆成「十彐女」時，兩個字根一體寫就；拆成「コ女」時，只有一個字根一體寫就。故而拆成「十彐女」更符合筆順。
...
宇浩拆分中，**有且只有**以下兩種情況可以斷開筆畫：

1. 「<!-- verbatim -->我識羲<!-- verbatim -->」等字中「戈」的橫，可以向左斷開。
...
✅
<CharPlot char='的' :parts='[5,2,1]' :colors='[3,2,1]' />
❌
</div>
...
1. 嚴格按照規則，並接受一些**感性**和**理性**之間的矛盾衝突。
1. 對部分漢字設置特例，調整其拆分方法，以符合**更多人**的感性認知。

宇浩拆分，選擇了第一種策略。這是因爲，我們對於常用字的直觀體驗，無法完美地延伸到大字集中。當面對**繁簡通打**和**大字集**時，設置特例會帶來極大的不確定性和混亂。因此，我們只能接受部分漢字的拆分結果可能不夠直觀這一事實。

這種策略的選擇，主要基於宇浩系列輸入法的定位，而不是普世的。對於着些著眼於**小字集**輸入法，設置部分特例，調整拆分方法，可能是更合適的選擇。
...
- 首先排除「䒑丿」。原因是「䒑」字根中的點應當和橫相連（艹字頭的行書寫法），而「亚」字中兩點分散了，這違反了[字根的內在屬性](./intrinsic)。
- 「丷一」是三根，故而「一业」勝出。

<div class="flex justify-left flex-wrap">
<CharPlot char='亚' :parts='[1,5]' />
✅
<CharPlot char='亚' :parts='[3,3]' />
❌
<CharPlot char='亚' :parts='[3,2,1]' />
❌
</div>
...
- 首先排除「䒑丿」。原因是「䒑」字根中的點應當和橫相連（艹字頭的行書寫法），而「严」字中兩點分散了，這違反了[字根的內在屬性](./intrinsic)。
...
「失」，候選拆分「丿夫」「牛丶」。它們都是兩根，都相連。但「牛丶」中「牛」的一豎寫作了撇，違反了[保持原形](./division#保持原形)規則，故而「丿夫」勝出。

<div class="flex justify-left flex-wrap">
<CharPlot char='失' :parts='[1,4]' />
✅
<CharPlot char='失' :parts='[4,1]' />
❌
</div>
...
<div class="flex justify-left flex-wrap">
<CharPlot char='午' :parts='[1,3]' />
❌
<CharPlot char='午' :parts='[2,2]' />
✅
</div>
...
「重」，候選拆分「丿<!-- verbatim -->車<!-- verbatim -->一」和「千里」。

「千里」是兩根，滿足「字根最少」，直接雖然中間一豎斷開，但依然勝出。

| 拆分方案 | 丿<!-- verbatim -->車<!-- verbatim -->一 | 千里 |
| -------- | ---------------------------------------- | ---- |
| 字根最少 | 3 ❌                                      | 2 ✅  |
| 剩餘規則 | 跳過                                     | 跳過 |

<div class="flex justify-left flex-wrap">
<CharPlot char='重' :parts='[1,7,1]' />
❌
<CharPlot char='重' :parts='[2,4,1,2]' :colors='[1,2,0,2]' />
✅
</div>

---

「<!-- verbatim -->盡<!-- verbatim -->」，候選拆分「コ士灬皿」「乛龶灬皿」「彐丄灬皿」「肀一灬皿」四種。
...
<div class="flex justify-left flex-wrap">
<CharPlot char='盡' :parts='[1,1,1,2,4,5]' :colors='[1,2,1,2,3,4]' />
❌
<CharPlot char='盡' :parts='[1,4,4,5]' />
❌
...
「<ruby>曱<rt>yuē</rt></ruby>」和「<ruby>甴<rt>yóu</rt></ruby>」。
...
- 「曱」，候选拆分为「日丨」「口丅」「冂干」。
    ...
    ✅
    <CharPlot char='曱' :parts='[2,1,1,1]' :colors='[1,2,1,2]' />
    ❌
    <CharPlot char='曱' :parts='[2,3]' />
    ❌
    </div>
...
- 「甴」，候选拆分为「日丨」「囗丄」。两者都是两根，都結構完整，都有一個字根一體寫就。按笔顺向前取大，故而「日丨」勝出。
    ...
    ✅
    <CharPlot char='甴' :parts='[2,2,1]' :colors='[1,2,1]' />
    ❌
    </div>

---

「彧」，候選拆分「戈口一」「弋口一彡」「一口一乚彡丶」。

- 「戈口一」四根，「弋口一彡」四根，「一口一乚彡丶」六根。
- 首先排除「一口一乚彡丶」，雖然它全部字根一體寫就，但「字根最少」優先級最高。
- 「戈口一」和「弋口一彡」中，穿插寫就的字根數量相同，均爲一個「戈」。都有「相交」情況。
- 進入兜底規則「字根取大」。觀察到，「戈」的筆畫數大於「弋」，故而「戈口一」勝出。

| 拆分方案 | 戈口一 | 弋口一彡 | 一口一乚彡丶 |
| -------- | ------- | -------- | ------------ |
| 字根最少 | 4 ✅     | 4 ✅      | 6 ❌          |
| 結構完整 | ✅       |  ✅        | 跳過         |
| 符合筆順 | 三根  ✅ | 三根  ✅  | 跳過         |
...
有人問，為什麼不能拆成「戈口一」？這是因為按照漢字筆順，「口」出現於「」之前，根據[字根排序](#字根排序)的原則，最終的字根排序應該是「戈口一」。

<div class="flex justify-left flex-wrap">
<CharPlot char='彧' :parts='[1,3,1,1,1,2,1]' :colors='[1,2,3,1,1,4,1]' />
✅
<CharPlot char='彧' :parts='[1,3,1,1,1,2,1]' :colors='[1,2,3,1,4,4,1]' />
❌
