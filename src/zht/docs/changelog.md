...
本頁面展示宇浩系列輸入法更新記録。[下載安裝包請移步綱盤](http://zhuyuhao.ysepan.com/) 。
...
丙午年正月十五日上元，公曆二零二六年三月三日，宇浩系列輸入法 v3.11.0 正式版發佈。
...
其一：增加字根「」。此字根原拆爲「彐」，現不再分拆，單獨作爲一個字根使用。主要原因有三：

1. 「」拆「彐」後，不够直觀。很多用户都陸續反映過此問題。
1. 「」和「艮」字根形似，容易混淆。「食」字根作爲偏旁時的「飠」，下方就是「」，故而「」可以認定爲「艮」之變體。
1. 「」作爲「厶」之變體字根，只在此一處使用，無其他用途，「」拆爲「彐」似顯多餘，且「」字根本身亦不够直觀。

總上，「」字根單獨成爲一個字根，更加合理直觀。其在全系列輸入法中，皆歸併至「艮」字根下，與其完全同碼。
...
比如，原來「严」拆爲「一业丿」而不拆成「丷厂」，是因爲後者中，「丌」字根之「丿」變爲「丨」。俾使**豎·撇變形根**儘可能少，我們取「一业丿」。現在，「严」拆爲「丷厂」，因爲向前取大。

其三：删除字根「」，原因見上。

其四：删除字根「𠃎」（或顯示爲「㇘」），其名「豎彎折」。對於此字根之認定及其筆順，存在較嚴重之歧義：當作爲一順時針左拐之折筆（台灣標準偶見）邪，抑或作爲一豎加一橫（大陸標準基本如此）邪？實難判斷，故統一拆作「丨一」。

其五：删除字根「𠁁」。此字根之相關字皆不在常用字集内，專設根無甚必要，故删除之。其可直接拆爲「乚乛一一」。

---

根據社群長期之反饋建議，對**卿雲輸入法**部分字根進行回退，恢復以下字根及其編碼至 [v3.6.0](#_20240917-v3-6-0)：
...
- 使用新拆分註解引擎，優化拆分註解顯示效果，詞語不再顯示其拆分和編碼，而是顯示其中每個漢字的全碼。如果詞語超過五個字，則只顯示前四個和最後一個字之拆分和編碼，中間以省略號代替。
- 優化延遲頂字上屏腳本、過濾空格簡碼腳本。現在可以指定最大碼長，適用於其他長度之前綴碼方案。
- 優化韻碼提示器腳本。現在，輸入一碼和兩碼時，不再隱藏非韻碼上屏候選，而是將它們列在韻碼上屏字後面。爲第二個非韻碼上屏候選項備註「分號」。
- 優化部分 lua 腳本對於單字之判斷，正確處理變體選擇器。
- 其他優化。

### 2025 年 12 月 4 日 乙巳年十月十五日

乙巳年十月十五日，公曆二零二五年十二月四日，宇浩系列輸入法 v3.10.2 正式版發佈。

此次對部分 RIME 功能進行優化和修復，並且修正了一些漢字的拆分。

- 全系列數字直接上屏。數字後按下`.`會輸入英文句號，再次按下`.`會輸入中文句號。
- 日月空碼時不再頂出空格簡碼字詞。
- 修正 lua 錯誤：不在指定字集中的繁簡常用字並未處於第二優先狀態，而是放到了預測候選項之後。
...
### 2025 年 10 月 6 日 乙巳年八月十五日

天運歲次乙巳八月乙酉十五日戊申中元，公曆二零二五年十月六日，宇浩系列輸入法 v3.10.1 正式版發佈。

此次對部分 RIME 功能進行優化和修復，並且修正了一些漢字的拆分。

### 2025 年 9 月 6 日 乙巳年七月十五日

天運歲次乙巳七月甲申十五日戊寅中元，公曆二零二五年九月六日，宇浩系列輸入法 v3.10.0 正式版發佈。

---

拓展宇浩拆分及宇浩全系列輸入法的字符覆蓋以適配 [Unicode 17.0 標準](https://www.unicode.org/versions/Unicode17.0.0)：
...
- 「」字根編碼由`Dg`改爲`Sv`。受此影響，`就`代替`被`成爲`S`上的一級簡碼。
- 「了」字根編碼由`Ll`改爲`Vl`。受此影響，`了`代替`原`成爲`V`上的一級簡碼，`道`代替`了`成爲`L`上的一級簡碼。
- 「水氺」字根編碼由`Ns`改爲`Es`。受此影響，`没`代替`水`成爲`Ns`上的二級簡碼。
- 「<ruby>巳<rt>sì</rt></ruby>」字根編碼由`Os`改爲`Ns`。
- 「<ruby>曲<rt>sì</rt></ruby>」字根編碼由`Vq`改爲`Nq`。
- 「<ruby><rt>luò</rt></ruby>」字根編碼由`Dl`改爲`Sl`。
- 「」（第二筆是點或捺）字根編碼由`Ae`改爲`Je`。「二冫」（第二筆是撗或提）字根編碼不變。
...
其四，設「」〔忍甚切，ㄖㄣˊ，rén〕爲字根。

- 光華方案，捺區，視爲`羊`的附屬根。
- 星陳方案，視爲`羊`的附屬根。
- 卿雲方案，同`羊`，都在`M`上，取`rén`音。

注意到，「」偏旁並不是<ruby>羊<rt>yáng</rt></ruby>字的異體，不同字型中寫法不同（`䒑丨`或`八丅`或`儿下`），出現頻率不高（只在`鬲`相關字中出現），且影響其他折分（`岡`需改拆「」凵），故不單設字根。
...
其六，由「」字根替代「黽𪚴」等字根，防止由`龜𪓝𪚳𪚭𪚰𪓑`類字産生大量隱藏歸併根。「」字根編碼爲`Jv`。

其七，移除<ruby>韋<rt>weí</rt></ruby>字根。其雖爲康熙部首，但出現頻率不高，且對應簡化字`韦`無字根。故`韋`字皆拆`𫝀口㐄`。卿雲方案中，「<ruby><rt>wǔ</rt></ruby>」字根編碼由`A`改至`K`。
...
- 字根「」小碼改爲`i`，因其不讀`chén`，而是<ruby>頤<rt>yí</rt></ruby>字的異體，讀`yí`。
- 字根「」小碼改爲`v`，因其無讀音。此二字根不再同`卜⺊`歸併。
...
- [譚淞宸](https://github.com/tansongchen)：CJK 基本區和擴展 A 區`夨髙䕎夓宺`等相關字。增設「」字根。
- 油甘子：`𩒞`
- 橢暶：`𰁙`
...
- 增加繁體常用部件「」爲字根，分離以下繁簡常用字重碼`渙涣瘓痪换換喚唤焕煥殃`。繁簡通打選重率顯著降低。光華編碼`Yv`，星陳編碼`Xv`，卿雲編碼`Ov`，其大碼都同`冖⺈`，小碼都爲`v`。
- 糾正大字集部分錯誤拆分。
