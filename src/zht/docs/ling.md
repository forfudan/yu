...
相同點：都是五碼定長、全字集、繁簡通打。字根都是要麼兩碼要麼三碼。
...
不同點：鄭碼首根可能取到三碼；日月最多取兩碼。鄭碼當主根多取一碼的時候，會少取第二根；日月當主根多取一碼的時候，會少取第三根。鄭碼的主根后需要補`A`並且加空格上屏；日月的補碼（韻碼）同時起到了補充和上屏的作用。

### 真碼輸入法

相同點：都是字根雙編碼，首根都可能取到兩碼。當主根多取一碼的時候，都會少取第三根。

不同點：真碼的主根后需要補`I`並且加空格上屏；日月的補碼（韻碼）同時起到了補充和上屏的作用。
...
其中，A區按鍵數量爲20（輔音字母），B區按鍵數量爲5（元音字母）。計算可得編碼空間為 202100（二十萬左右），是傳統四碼定長方案的一半左右。
