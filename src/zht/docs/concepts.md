...
「拼音輸入法」，又稱「字音輸入法」「音碼輸入法」「拼音」「音碼」等，是指通過漢字的「讀音」來構建編碼的輸入法。這類輸入法通常基於某類拼音系統，如漢語拼音、注音、方言拼音、音標等。

```txt
漢字 --> 讀音 --> 編碼
```

純粹的音碼（**純音**），不依賴除讀音外的其他信息的。也就是説，當出現一個新漢字時，只要知道它的讀音，就可以確定它的完整編碼；其他的信息，包括漢字的形態、漢字的含義等，都不影響編碼的確定。
...
字形輸入法，又稱「形碼輸入法」「形碼」等，是指通過漢字的形態特徵來構建編碼的輸入法。所謂形態特徵，指的是部首、字根、筆劃、結構等。

一般地，一個漢字會被分解爲若干字根或筆劃，每一個字根或筆劃對應一個或多個編碼字符。

```txt
漢字 --> 部首/字根/筆劃/結構 --> 編碼
```

純粹的形碼（**純形**），不依賴除字形外的任何其他信息。也就是説，當出現一個新漢字時，只要知道它怎麼寫，就可以確定它的完整編碼；其他的信息，包括漢字的讀音、漢字的含義等，都不影響編碼的確定。
...
- 五筆字形輸入法：字根、結構特徵
- 倉頡輸入法：部首、形態特徵。
- 鄭碼、徐碼、虎碼、宇浩系列輸入法、天碼：字根特徵。

:::

::: warning 辨析

部分純形輸入法中，形態特徵（部首/字根/筆劃/結構）的編碼有可能選取自該形態特徵的慣用讀音。但衹要不涉及整個漢字的讀音，則仍屬於純形輸入法。

比如，某輸入法中，「甜」字可以依據字形拆分爲「舌」「甘」兩個字根，以及左右結構。假設「舌」的編碼是`she`、「甘」的編碼是`gan`、左右結構的編碼是`zuoyou`，則「甜」字的編碼爲`sheganzuoyou`。因爲此編碼不依賴於「甜」字的讀音，此輸入法仍然是**純形**的。
...
音形輸入法以音爲主，以形爲輔，部分情況下字形的部分只作爲補充信息而被省略。
...
形音輸入法以形爲主，以音爲輔，部分情況下字音的部分只作爲補充信息而被省略。
...
1. 北京師範大學邢紅兵的 [《25 亿字语料汉字字频表》](https://faculty.blcu.edu.cn/xinghb/zh_CN/article/167473/content/1437.htm)：反映了大陸地區現代漢語的字頻情况，用字較爲正式官方。基本簡化字爲主，有部分繁體字出現。
...
1. 臺灣地區教育部門的[《字頻總表》](https://language.moe.gov.tw/001/Upload/files/SITE_CONTENT/M0001/PIN/biau1.htm?open)：反映了臺灣地區的現代漢語字頻情況，全部爲繁體字。
...
只包含多字詞詞頻的數據，不能反映多字詞的實際頻率，它是一種**條件概率**。這是因爲多數文本無法被多字詞完全分割。
...
GBK，又稱《汉字内码扩展规范(GBK)》，是中國大陸於1995年頒布的漢字編碼標準。它包含21003個漢字，包含簡體和繁體字，爲歷史字集。

#### GB18030

GB18030，又稱《信息技術　中文編碼字符集》，是中國大陸於2000年頒布的漢字編碼標準。當前標準爲GB18030-2022。

#### 中日韓統一漢字CJK

CJK，又稱《中日韓統一漢字》，是Unicode萬國碼中定義的漢字集。它包含了常用字和生僻字，並且分爲若干個擴展區域，總計包含10萬左右的漢字。
...
`见到一个人` 中有兩個簡化字，三個傳承字，其對應的傳統寫法爲 `見到一個人`。廣義上，我們可以説這五個字都是簡體字（簡體中文）；狹義上，我們只認爲「见」「到」「个」是簡體字。
...
`見到一個人`这五個字，狹義上，我們認爲`見` `個`是繁體字；廣義上，我們可以説這五個字都是繁體字（繁體中文）。
...
1. 狹義的異體字，不包括對應的正體字。
1. 廣義的異體字，包括對應的正體字，也就是説這些字互爲異體字。
...
1. 五筆字形的編碼集合爲 `a-y` 的 25 個字母。
1. 冰雪清韻的編碼集合爲 `a-z` 的 26 個字母，加上 `;,./` 4个符號，共30個字符。

:::

### 碼位

碼位，又稱「編碼位置」，指的是一個輸入方案中的一個合法的編碼，也就是説它是按照此輸入法的編碼規則可以取到的編碼。

::: tip 舉例

五筆字形的編碼字符集合包括`a-y`的25個字母，且它的最長編碼長度爲 4，因此它的碼位是任何四個`a-y`字母的組合。比如 `abcd`、`efgh`、`ijkl` 等。
...
全拼輸入法：編碼空間包括所有合法的漢語拼音不帶調音節，如 `pian`、`xian`、`hao` 等。`pian` 是合法的編碼，而 `pianx` 則不是合法的編碼。因此全拼的編碼空間不包括 `pianx`。它的編碼空間大小約爲 400 左右。

宇浩星陳方案：第一碼爲`a-y`的25个字母，其餘編碼爲`a-z`的26个字母，最大碼長爲4，因此它的編碼空間大小爲 $25 \times 26^3 = 1757600$。

宇浩日月方案：末碼必須以`aeiou`等元音字母結尾或者達到最大碼長5，韻碼前可以有若干輔音字母，首碼不得爲`z`，故而編碼空間大小爲：
$$
\begin{aligned}
...
實際編碼，指的是用户在使用輸入法時實際輸入的編碼。有時候，爲了消除歧義，實際編碼會與理論編碼有所不同，比如使用額外的分隔符。

::: tip 舉例

「西安」在全拼輸入法中的的理論編碼爲 `xian`，實際編碼可能是 `xian`（不進行手工分割），也可能是 `xi'an`（使用了單引號作爲分隔符）。

:::

### 碼長

碼長，又稱「編碼長度」，指的是某輸入法中，某一漢字或詞語的編碼字符的數量。碼長分爲兩類：

1. **理論碼長**：指的是輸入法在設計時所定義的編碼方式的最大長度。
1. **實際碼長**：指的是用户在使用輸入法時實際輸入的編碼字符的數量。由於需要唯一確定某個漢字，用户需要額外輸入分割符、確定符、選重符等字符，因此實際碼長可能會大於理論碼長。

### 平均碼長

平均碼長，是指某輸入法中，輸入一個漢字或字符所需要的平均**實際**編碼長度。

一般地，當輸入方案作爲修飾詞時，「平均碼長」可以簡稱爲「碼長」。

::: warning 辨析

形碼輸入方案中，爲了消除歧義，往往需要在實際編碼後添加分隔符號（如空格鍵）或者選重複號。因此，平均碼長通常大於理論碼長。爲較爲精確地計算平均碼長，一般會採取以下手段：

1. 在未達到最大碼長的情況下，爲編碼碼長添加1。
1. 在出現重碼的情況下，爲非首選項的編碼碼長添加1。
...
連續編碼 `nihaoshijie` 時，編碼可能會被分割爲 `ni  hao  shi  jie` 四個部分，也可能被分割爲  `ni  hao  shi  ji  e` 五個部分。兩種分割方法都是合法的。

:::

#### 自定

自定，又稱**自確定**、**自分割**、**内頂**、**内生性分割**、**内向性分割**，是指當前漢字的編碼自身可以確定它的結束，而不需要額外分隔符號或後續漢字編碼來確定當前漢字的編碼結束。

::: warning 辨析

例如，某輸入法中的所有漢字編碼的最後一碼都必然是`a`，且`a`只作爲末尾編碼，則它具有自定屬性。當出現`a`時，就可以確定當前漢字編碼的結束。不需要看到後續漢字的編碼。

:::

::: tip 方案舉例

注音輸入法：如大千注音輸入法。由於注音符號的最後一碼是聲調符號，被分配在一個單獨的編碼字符集合中，因此它具有自定屬性。

帶調拼音輸入法：如地球拼音輸入法。由於拼音的最後一碼是聲調符號，被分配在一個單獨的編碼字符集合中，因此它具有自定屬性。

日月輸入法：純形碼。將字根的最後一碼設計爲字根元音在五個元音字母`aeiou`上的映射，這樣當出現元音字母時，就可以確定當前編碼的結束。

靈明輸入法：純形碼。將字根的最後一碼設計爲字根元音在五個元音字母`aeiou`上的映射，這樣當出現元音字母時，就可以確定當前編碼的結束。

松煙：純形碼。將字根的最後一碼設計爲集合`weruio`中的字母，這樣當出現該集合字母時，就可以確定當前編碼的結束。

:::

#### 前綴碼

前綴碼，又稱**自定碼**、**自分割碼**、**前置碼**、**前綴無關碼**。是指任何一個漢字的編碼都不是任何其他漢字編碼的前若干碼。

前綴碼是自定的、編碼分割是内向性的，也就是説當前編碼自身可以確定它的結束，而不需要看到下一個有效編碼。

前綴碼通常是**末尾固定位置若干碼**和其他編碼在不同的編碼字符集合中的編碼的前若干碼。

例如，假設存在兩個編碼字符集$A$和$B$，它們的交集爲空集，則前綴碼的一類編碼形態爲：$B$, $AB$, $AAB$, $AAAB$, $AAAAB$，以此類推。如此，觀察到$B$集合的字符時，便可以確定當前漢字編碼的結束。

另一類編碼形式爲：$BBA$, $ABBA$, $AABBA$, $AAABBA$，以此類推。如此，觀察到$BBA$模式時，便可以確定當前漢字編碼的結束。

::: warning 辨析

如果一個方案中所有的編碼都是以空格鍵結尾的，且空格鍵只作爲結尾，則它具有前綴碼屬性。

前綴碼具有分割唯一屬性。

**任何**限長碼或變長碼，如果在其編碼後附加額外的分隔符號（如空格），則其編碼具有自定的前綴碼屬性。此時，空格可以看作它實際編碼的最後一碼，出現空格時，就可以確定當前漢字編碼的結束。
...
**任何**限長碼或變長碼，在未達到其最大長度時，如果在其編碼後附加額外的分隔符號（如空格）或標點符號（如句號），則其編碼具有頂功碼屬性。此時，分隔符號（如空格）可以看作下一個空白字符的實際編碼；標點符號（如句號）可以看作下一個標點符號的實際編碼。當出現分隔符號或標點符號時，就可以確定當前漢字編碼的結束。

舉例一：某方案中，一個漢字的第一個編碼，被設計爲左手的按鍵，而剩余的編碼被設計在右手上。這樣的編碼是否具有頂功屬性？

這樣的編碼方式具有頂功屬性。因爲每當看到左手的按鍵時，就可以確定這是一個新漢字的開始碼。

舉例二：某方案中，一個漢字的前兩碼都是輔音字母，而剩余的編碼是元音字母 `aeiou`。這樣的編碼是否具有頂功屬性？

這樣的編碼方式具有頂功屬性。因爲每當看到兩個輔音字母時，就可以確定這是一個新漢字的開始碼。否則，它永遠都是前一個漢字的後續碼。
...
這種情況，我們需要往前看三碼方能判斷編碼的分割情況。這個方案稱爲「五二頂」，也就是觀察第五碼來判斷前兩碼是否可以唯一分割并上屏。

由於在第五碼之前，無法確定當前編碼是當前漢字的編碼還是前序漢字的後續編碼，因此這個方案應歸爲頂功碼。

:::

::: tip 方案舉例

逸碼：純形碼，頂功碼。設兩個不相交的字符集合$A$和$B$：首碼必爲$A$，第二碼可$A$可$B$，其後各碼皆爲$B$。編碼模式包括：$AA$, $AB$, $AAB$, $ABB$, $AABB$, $ABBB$, $AABBB$, $ABBBB$ 等。因爲任意數量編碼模式連綴非其他任意數量編碼模式連綴，故而具有頂功屬性。衹要出現$BA$模式，就可以確定前序漢字編碼的結束。

C42輸入法：形音碼，頂功碼。編碼模式爲：$AA$, $AAAB$ 等。因爲任意數量編碼模式連綴非其他任意數量編碼模式連綴，故而具有頂功屬性。由於$AA$是$AAAB$的前綴，故而不是前綴碼。
部分編碼分割呈現前綴碼和頂功碼特徵：衹要出現$B$模式，就可以確定本漢字編碼的結束；衹要出現$AAAA$模式，就可以確定前序漢字編碼的結束。

[徐碼五二頂](https://github.com/Ace-Who/rime-xuma)：純形碼，頂功碼。編碼模式爲：$AA$, $AAAB$, $AAAAB$ 等。因爲任意數量編碼模式連綴非其他任意數量編碼模式連綴，故而具有頂功屬性。由於$AA$是$AAAB$的前綴，故而不是前綴碼。
...
限長，指的是每個漢字的編碼**最大**長度固定。也就是説，無論輸入什麼漢字，編碼的最大長度都是一樣的。當出現一個新的漢字時，它的最大長度是確定的。

最大長度爲 N 的限長方案稱爲「N碼限長」方案，簡稱「N限」方案。

::: warning 辨析

限長碼不一定具有「分割唯一」的屬性。

限長碼通過添加分隔符號（如空格），可以在實際編碼中達到「分割唯一」的屬性。

在中文輸入法中，最大長度爲 N 的限長方案往往被稱爲「N碼定長」方案，簡稱「N定」方案。

:::

::: tip 方案舉例

五筆字形：全碼最短編碼長度爲3，最長編碼長度爲4。它是四碼限長方案。
倉頡：全碼最短編碼長度爲1，最長編碼長度爲5。它是五碼限長方案。
宇浩日月：全碼最短編碼長度爲2，最長編碼長度爲5。前綴碼，兼具免空格自分割特徵和限長特徵。它是五碼限長方案。

:::

#### 定長

狹義上的定長，指的是每個漢字的編碼長度固定。也就是説，無論輸入什麼漢字，編碼的長度都是一樣的，又稱爲[絕對定長](#絕對定長)。

廣義上的定長，指的是每個漢字的編碼**最大**長度固定，同[限長](#限長)。
...
- 廣義上，指的是編碼長度不固定的輸入法。也就是説，輸入不同的漢字，編碼的最大長度可能會有所不同。
- 狹義上，指的是編碼長度不固定且沒有上限的輸入法。也就是説，輸入不同的漢字，編碼的最大長度可能會有所不同，且沒有最大長度限制。當出現一個新的漢字時，它的最大長度是不確定的。
...
拆分，是指字形輸入法中，根據若干預設規則，將一個漢字分解爲若干部件的有序組合的過程。

拆分不一定是唯一的。也就是説，對於同一個漢字，可能存在多種不同的拆分方式。

拆分不一定是覆蓋的。也就是説，拆分後的部件不一定能完全覆蓋漢字的所有特徵。

### 拆分規則

拆分規則，是指將漢字拆分爲若干部件的具體方法。一般而言，拆分規則包含若干排除規則和優先級，將所有的拆分可能的序列進行排序，並選擇最優的拆分序列。
...
字根的數量是有限的，任何新的漢字都可以拆分爲字根集合中元素的有序組合。

根據字根的數量，漢字可以分爲多種類型。

#### 單根字

單根字，又稱「字根字」「成字字根」，是指拆分爲單個字根的漢字。
...
宇浩日月：字根大多數三編碼，分爲大碼、聲碼、韻碼。聲碼有時省略。
...
**主根**，編碼數量爲一，字根字需要補充編碼。或者編碼數量爲二，但小碼的編碼字符是固定的。
**副根**，編碼數量大於一，字根字不需要補充編碼。

部分輸入法會將主根繼續分爲「第一主根」和「第二主根」，並將它們的編碼方式區分開來。

單字編碼時，主根作爲首碼，則不需要補充編碼（小碼）；副根作爲首碼，則需要補充編碼（小碼）。

:::

::: tip 方案舉例

鄭碼輸入法：部分字根是主根，它們的編碼只有一個字符。爲了輸入字根字，必須補充編碼。比如「言」是 `S` 上的第一主根，輸入「言」需要補充編碼 `a`，因此「言」的編碼是 `Sa`。

徐碼輸入法：部分字根是主根，它們的小碼統一爲`v`或`u`。比如「石」是 `G` 上的第一主根，編碼是 `Gv`。再比如「一」是 `G` 上的第二主根，編碼是 `Gu`。

:::

### 大小根

大小根，是指部分字根的編碼方式一致，但**參與單字編碼的方式**和其他字根不同。它是「主副根」的一種特殊情況。

::: warning 辨析

大小根的小碼編碼規一般是一致的，比如都是音托。只是在參與單字編碼的時候，主根作爲首碼，則不需要補充編碼（小碼）；副根作爲首碼，則需要補充編碼（小碼）。

:::

::: tip 方案舉例

靈明輸入法：所有字根的聲碼和韻碼都是音托，但部分字根是大根，部分字根是小根。大根在任何位置都不需要取聲碼；小根在首根或末根位置需要取聲碼。
...
鄭碼輸入法：`見见`二字字形不同，但音義相同，後者爲前者的簡化。因此它們被歸併到「见」字根上，賦予相同的編碼`Lr`。

虎碼輸入法：`八丷`二字字形不同，但音義相同，後者爲前者的古字。因此它們被歸併到「八」字根上，賦予相同的編碼`Hb`。
...
字根聚類，指的是將若干個字根按照某種内在特徵劃分爲一組，並賦予相同的主要編碼（第一編碼、大碼）。
...
早期的形碼大多具有強聚類特性。他們基於字根的形態來設計，將相似形態的字根放在同一個大碼上，便於用户記憶。聚類的組數略大於編碼字符集的大小。

:::

::: tip 方案舉例

- 鄭碼輸入法：字根大碼相同的字根大多具有相似的起筆形態，爲強聚類。比如`S`大碼的字根都具有「亠」的形態。
- 星陳輸入法：部分形態相似的字根被聚類到同一個大碼上。比如「方文夂攵亦亡亥豕」等字根的大碼都是`T`。
- 天碼輸入法：字根大碼相同的字根大多和此編碼字母的形態有關，爲強聚類。比如`A`大碼的字根都具有三角形的形態，如「人八金又亽」等字根。
...
「托」字有「凭借、依靠」的意思，在輸入法領域，通常引申爲「提示」義。依靠拼音字母，可以方便用户迅速掌握小碼，避免額外的學習成本。
...
鄭碼：「辛」字根的大碼爲 `S`，因爲它的下半部分具有「十」的形態，因此小碼選用「十」字根的大碼`E`。辛字根的編碼故而爲 `Se`。
...
一般地，輸入法的理論全碼不包括分隔符號。實際輸入中，爲了消除歧義，可能會在全碼後添加分隔符號（如空格鍵）。我們也可以認爲這些分隔符號是全碼的一部分。
...
一般地，輸入法的理論簡碼不包括分隔符號。實際輸入中，爲了消除歧義，**多數情況下**會在鍵碼後添加分隔符號或選重符號，我們也可以認爲這些分隔符號是簡碼的一部分。
...
一級簡碼，指的是長度爲一的簡碼。對應的字稱爲「一級簡碼字」，對應的詞稱爲「一級簡碼詞」。

::: warning 辨析

由於分割符號的必要性，一級簡碼的實際編碼長度一般爲二。因此，對於四碼定長方案，一級簡碼**節約**的編碼長度爲2。

:::

::: tip 方案舉例

五筆字形：橫區和豎區十鍵上的一級簡碼字爲「一地要在工，上是中国同」。

:::

#### 二級簡碼

二級簡碼，指的是長度爲二的簡碼。對應的字稱爲「二級簡碼字」，對應的詞稱爲「二級簡碼詞」。

::: warning 辨析

由於分割符號的必要性，二級簡碼的實際編碼長度一般爲三。因此，對於四碼定長方案，二級簡碼**節約**的編碼長度爲1。

:::

#### 三級簡碼

三級簡碼，指的是長度爲三的簡碼。對應的字稱爲「三級簡碼字」，對應的詞稱爲「三級簡碼詞」。
...
$N$級簡碼覆蓋率，指的是碼長爲$N$的簡碼和全碼在某一文本空間下的出現頻率。它符合以下公式：

$$

N級簡碼覆蓋率 = \sum_{i} (碼長爲N的簡碼和全碼字_i \times 該字出現頻率_i)

$$

::: warning 辨析

一般地，二十六鍵四碼限長方案的二級簡碼覆蓋率約在70%左右。
...
鄭碼輸入法：當第一根爲副根時，簡碼可能會跳過此副根的小碼（位碼），而取第二根的大碼（區碼）。因此，鄭碼輸入法不具有全簡一致性。

宇浩日月：本方案有兩類簡碼。一類簡碼爲全碼前幾碼加末根韻碼，因爲韻碼具有分隔符號作用，因此其理論簡碼和實際簡碼一致，不具有全簡一致性。另一類簡碼爲全碼前幾碼，需要空格分隔符，因此其理論簡碼具有全簡一致性。
...
鄭碼輸入法：當一個詞語中的某個字的第一根爲副根時，詞語編碼可能會跳過此副根的小碼（位碼），而取此字第二根的大碼（區碼）。因此，鄭碼輸入法不具有字詞一致性。
...
1. 音碼輸入法中，因爲某些漢字有多個讀音，因此它們有多個編碼選項。
1. 形碼輸入法中，因爲某些漢字有多種字形，因此它們有多個編碼選項。
1. 形碼輸入法中，因爲某些漢字有多種拆分方式，因此它們有多個編碼選項。
...
倉頡輸入法：使用`難`（對應編碼爲`X`）來表示一些複雜的部件，比如「身」字的編碼爲`竹難竹`。
...
徐碼：如果一個字的首根爲副根且字根數大於一，則此字的編碼第四碼必爲該字首根的小碼。如此，編碼的第二碼保證爲次根的大碼。

宇浩星陳：如果一個字的編碼不足4，則在末尾補充首根的小碼。

天碼：如果一個字的編碼不足4，則在末尾補充首根的小碼（小碼爲`v`的不用補充）。
...
倉頡輸入法：使用`重`（對應編碼爲`Z`）來區分同一個編碼對應的多個漢字。一般地，選重碼賦予字頻較低的漢字。選重碼出現在編碼的開頭。

比如：「知」「佑」的編碼都是`人大口`（`OKR`），此時，在低頻字「佑」的編碼前添加選重碼，使得「佑」的編碼也可以爲`重人大口`（`ZOKR`）。

:::

### 識別碼

識別碼，指的是輸入法中，爲了更精確地識別和定位漢字而添加的一個或多個編碼字符。識別碼往往取用額外的信息，比如結構、末筆、讀音、含義等。

::: tip 方案舉例

五筆字形：識別碼採用漢字的末筆和結構雙重信息。當根據字根映射形成的單字編碼長度不足4時，會使用結構信息和末筆信息的結合來補充一碼用於進一步識別該字。比如：「和」字拆分爲「禾口」，對應編碼是`TK`，不足4碼，故而補充一碼識別碼。這一碼取結構信息（左右結構）和末筆信息（橫）的組合，爲`G`（11鍵）。因此它的全碼爲`TKG`。

徐碼：當字的第一根爲副根，且該字編碼不足4時，會使用首根的小碼作爲識別碼。
...
補碼的目的可以是爲了補充編碼的長度，也可以是爲了補充編碼的特徵從而降低編碼衝突可能性。
...
倉頡輸入法：繁簡通打。因爲它的拆字方式是通過切割和提取漢字特徵，因此可以被動地離散繁簡字編碼。

徐碼輸入法：繁簡通打。因爲它將對應的繁簡字根安排在不同的大碼上。

宇浩系列輸入法：繁簡通打。因爲它對於繁簡混合字頻進行優化。

天碼輸入法：繁簡通打。因爲它字根安排在和鍵位拉丁字母相似的按鍵上，因此可以被動地離散繁簡字編碼。
...
聯想，指的是在輸入法中，輸入軟件根據統計模型或語言模型，對用户輸入的編碼分割、編碼對應的字詞、錯誤輸入進行及時調整。

### 預測

預測，指的是在輸入法中，輸入軟件根據當前輸入的編碼字符，預測用户可能輸入的下一個漢字或詞語。

### 模糊鍵

模糊鍵，指的是在輸入法中，對於某些編碼字符的輸入，允許用户輸入一個特定的編碼字符來匹配多個可能的漢字或詞語。
...
出簡不出全，指的是在輸入法中，當一個漢字存在簡碼時，刪除它的全碼，僅保留簡碼。

### 繁簡轉換

繁簡轉換，指的是在輸入法中，將輸入的繁體字或簡體字轉換爲對應的簡體字或繁體字。

### 上屏方式

#### 整句上屏

整句上屏，是指輸入法在用户輸入多個漢字後，將這些漢字一起上屏。

隨著信息技術的不斷發展，整句上屏往往配合輸入聯想功能，通過隱馬爾可夫鏈、LLM等技術，實現對整句的預測。

#### 唯一上屏

唯一上屏，又稱**唯一候選上屏**，指的是當前輸入的編碼可以唯一確定爲一個漢字（候選項爲一）時，直接上屏該漢字。

唯一上屏的觸發條件一定是内生的。也就是説，它不依賴下一個漢字的編碼來確定當前漢字編碼的結束。它包括：
...
一個輸入法的編碼最大碼長爲4，當用户輸入的編碼爲`abcd`，且這個編碼對應了唯一字符（如`漢`），則直接上屏`漢`。

:::

::: tip 方案舉例

三碼鄭碼（六碼除重模式）。該模式下，編碼爲不定長狀態。當用户輸入的單字編碼構成唯一的漢字候選項且无後續預測（未成詞），則直接上屏；當用户輸入的單字編碼參與了後續預測（成詞），則不直接上屏，而是等待用户輸入更多的編碼字符判斷是否上屏詞語或是單字。
...
空格鍵本身不上屏任何符號，而是將一個方案的實際編碼轉換爲**前綴碼**。因此，空格上屏可以視爲一種特殊的「唯一確定上屏」。

#### 選重上屏

選重上屏，是指在用户輸入編碼後，通過選擇候選字來確定上屏的漢字。

選重鍵本身不上屏任何符號，而是將一個方案的實際編碼轉換爲**前綴碼**。因此，選重上屏可以視爲一種特殊的「唯一確定上屏」。但這種所謂「唯一確定」是在實際輸入過程當中及時更新的，不具備通用的預見性。

#### 延遲上屏

延遲上屏，又稱「頂字上屏」，是指在用户輸入編碼且達到唯一分割條件時，不立即上屏，而是等待用户繼續輸入更多的編碼字符後再上屏。

延遲上屏時，候選項可能唯一，也可能不唯一。

延遲上屏的觸發條件可以是内生的，也可以是外生的。也就是説，它既可以不依賴下一個漢字的編碼來確定當前漢字編碼的結束，比如：
...
延遲上屏可以防止用户輸入全碼後自動上屏，從而避免輸入錯誤無法回改的情況。
...
RIME 輸入法將 `auto_select` 設置爲 `false`，則在輸入編碼後不會立即上屏，而是等待用户繼續輸入一個字符後上屏前字。

:::

#### 頂功上屏

頂功上屏，是指在用户輸入編碼時，一旦編碼模式構成了唯一分割，則直接上屏其對應的候選漢字，並且保留剩餘編碼在輸入區。
...
任何限長方案，在未達到其最大編碼長度時，在後方添加標點符號使候選漢字和標點同時上屏，同時具有頂功上屏和唯一上屏的特徵。這是因爲漢字本身未達到唯一分割條件，看到後續字符（標點符號）後，纔確定了唯一分割條件，此時，漢字的上屏就是一種頂功上屏。而標點符號本身是自分割的，所以標點符號的上屏是唯一候選上屏。
...
符號鍵本身不僅上屏前字，同時也上屏了符號本身。因此它可以根據具體情況視爲以下上屏方式的一種特例：
...
離散水平有多個層級，比如避免完全相同的拆分、將容易形成相似組合的字根分散到不同的編碼位置、在原始編碼後添加額外信息作爲識別碼等。

:::

### 選重

選重，是指在輸入法中，當用户輸入的編碼對應多個候選漢字時，通過選擇候選漢字來確定上屏的漢字。選重方法包括數字選擇、符號選擇、鼠標選擇等。
...
假設 $Z$ 爲一個漢字的集合，$M$ 爲一個編碼的集合。漢字 $z_{ij}$ 定義爲編碼爲 $m_i$ 的第 $j$ 個漢字，順序隨機。那麽，靜態重碼數可以表達爲：

$$N_{s} = \mid \{z_{ia}, z_{ib}  \text{ if } M(z_{ia}) = M(z_{ib}) \text{ for all } a,b \in J_i \text{ and } i \in I \}.$$

以下爲 Python 代碼示例，使用 numpy 包計算靜態重碼數：
...
單字靜態選重數，指的是在一個字符集中，**編碼完全相同、且不爲首選的漢字數量**。以下等式成立：
...
單字靜態選重率，是指在一個字符集中，**編碼完全相同、且不爲首選的漢字數量**與**字集包含字數**之比。它可以表達爲：
...
單字動態選重率，又稱「動重」「動態重碼率」「字頻加權選重率」，是指編碼完全相同、且不爲首選的漢字的出現頻率。它可以表達爲：
...
「動態」一詞在輸入法領域内共識性地表達了「加權」的含義。它與「靜態」的區別在於，動態選重率考慮了漢字的出現頻率，而靜態選重率則不考慮。

:::

::: details 動態選重率的計算公式和代碼

假設 $Z$ 爲一個漢字的集合，$M$ 爲一個編碼的集合，$p:Z\rightarrow [0,1]$ 爲漢字到其出現頻率的映射。用編碼和字頻對漢字排序，使漢字 $z_{ij}$ 是編碼爲 $m_i$ 的第 $j$ 個漢字，$i \in I$, $j \in J_i$, 且滿足 $a\geq b$ 時，$f(z_{ia})\geq f(z_{ib})$。
動態選重率可以表達爲：
...
詞語動態選重率，又稱「詞重」「詞語重碼」「詞頻加權選重率」「詞語動態重碼率」「理論詞語選重率」，是指在某一詞語集合内，編碼完全相同、且不爲首選的詞語的**理論**出現頻率。它可以表達爲：
...
多字詞動態選重率，有時也簡稱爲「詞重」「詞語重碼」「理論詞語選重率」，可以完整稱爲「詞語條件概率加權理論選重率」。它是指在某一詞語集合内，編碼完全相同、且不爲首選的多字詞語的**理論**出現頻率。它體現爲詞語動態選重率的條件概率。可以表達爲：
...
多字詞動態選重率和詞語動態選重率往往都簡稱爲「詞重」，然而兩者所反映和測量的維度并不相同。因爲給定任意文本，無法將其分爲只包含多字詞的序列。因此：
...
如果用户人工分詞的結果和詞語集合完全一致，則實際詞語選重率和多字詞動態選重率相等。

如果用户人工分詞的結果較詞語集合保守（使用很少的多字詞），則實際詞語選重率會接近單字動態選重率。

因此，以下關係成立：

$$
\begin{align*}
\lim_{用户分詞 \to 詞語集合} 實際詞語選重率 &=  理論詞語動態選重率 \\
\lim_{用户分詞 \to 0} 實際詞語選重率 &=  單字動態選重率
\end{align*}
$$
...
我們得到了 14 個詞語，也就是説，用户要輸入 14 次全碼。假設「請」和「描述」發生了重碼，那麽我們輸入這 14 個詞，就要選重一次。這句話的詞語動態選重率就是 1 / 13 = 7.1%。

情況二：如果詞庫爲**超大詞庫**，換言之，分詞比較激進，我們對這句話進行如下分割：

「請用·大約·十句話·來·描述·你所·居住·城市·歷史的·來龍去脈。」

我們得到了 10 個詞語，也就是説，用户要輸入 10 次全碼。假設「請用」和「描述」發生了重碼，那麽我們輸入這 10 個詞，就要選重一次。這句話的詞語動態選重率就是 1 / 10 = 10.0%。

情況三：如果詞庫爲**精簡詞庫**，換言之，分詞比較保守，我們對這句話進行如下分割：

「請·用·大約·十·句·話·來·描述·你·所·居住·城市·歷史·的·來·龍·去·脈。」

我們得到了 18 個詞語，也就是説，用户要輸入 18 次全碼。假設「請」和「描述」發生了重碼，那麽我們輸入這 18 個詞，就要選重一次。這句話的詞語動態選重率就是 1 / 18 = 5.56%。

從上例可以看出，詞語的實際重碼數據依賴於用户的人工分詞和統計數據中的詞語的一致性。在以下條件全部滿足時，實際動態選重率等同於理論詞語動態選重率：

- 用户只打詞頻表中出現的詞。如：詞頻表中若無「我們」，那麽用户只打「我·們」。
- 用户必定打長度較長的詞。如：出現「電視機」時，必定打「電視機」，而不是「電視·機」或「電·視·機」。這一條也意味着，一個多字詞若在字頻表中，那麽用户絕對不會打成單字。

這兩個條件一般不會實現，很多時候，用户會將某些詞頻表中的多字詞分爲單字詞輸入，這意味着：

實際輸入時的動態選重率，應介於「單字動態選重率」和「詞語完美動態選重率」之間。依照用户習慣：
...
速度當量，在輸入法領域可以無歧義地簡稱爲「當量」，指的是經過**標準化**之後，輸入一串編碼所**消耗的時間**。

一般地，速度當量由實驗測試得出。有以下主要步驟：

1. 測量按下某兩個按鍵所需要的時間**間隔**，得出平均值。比如按下`ad`的平均時間間隔爲 0.15 秒，按下`dk`的平均時間間隔爲 0.1 秒，按下`kq`的平均時間間隔爲 0.13 秒，按下`qz`的平均時間間隔爲 0.2 秒。
1. 找出所需時間最短的按鍵組合，並將其時間間隔設置爲**基準**，比如`dk`的時間間隔爲 0.1 秒，是幾組中最短的，故而將 0.1 秒設置爲基準。
1. 將其他按鍵組合的時間間隔除以基準時間間隔，得到**當量**。比如`ab`的當量爲 0.15 / 0.1 = 1.5，`dk`的當量爲 0.1 / 0.1 = 1.0，`kq`的當量爲 0.13 / 0.1 = 1.3，`qz`的當量爲 0.2 / 0.1 = 2.0。
...
速度當量也可以拓展到任何長度的編碼，只需將任意兩個連續編碼字符組的當量**相加**。記一個字的編碼 $B$ 中共有 $n$ 个字符（即 $B_1B_2...B_n$），位置 $i$ 和 $i+1$ 的一組按鍵的速度當量爲 $當量(B_i, B_{i+1})$，則有以下公式：

$$
編碼B速度當量 = \sum_{i=1}^{n-1} 當量(B_i, B_{i+1})
$$

例如，編碼 `adkqz` 的速度當量可以計算爲：$1.5 + 1.0 + 1.3 + 2.0 = 5.8$。
...
例如，編碼 `adkqz` 的鍵均當量可以計算爲：$(1.5 + 1.0 + 1.3 + 2.0)/(5-1) = 5.8 / 4 = 1.45$。

#### 輸入法加權鍵均當量

輸入法的加權鍵均當量，在無歧義的時候也可簡稱爲「輸入法的當量」，是將一個輸入法中所有編碼的鍵均當量，對其出現頻率進行加權平均。

假設一個輸入法共有 $M$ 可能的編碼，每個編碼 $K_i$ 對應（漢字）的出現頻率是已知的，則該輸入法的當量可以表達爲：
...
朱氏鍵位離散度，又稱*鍵位偏差值*，測量鍵位的**實際**使用頻率和**理想**使用頻率之間的差異，使用的是二階距離。則其公式爲：
...
其中，$\Delta_i = 按鍵實際頻率_i - 按鍵理想頻率_i$，$I$ 爲指示函數，當 $\Delta_i > 0$ 時，$I_{\Delta_i > 0} = 1$，否則 $I_{\Delta_i > 0} = 0$。$\gamma \in [0, +\infty]$ 是一個懲罰因子，用於調整高頻按鍵的懲罰程度。
