...
前綴碼的**末尾固定位置若干碼**（通常是末碼）和其他編碼須在不同的編碼字符集合中。

假設存在兩個編碼字符集$A$和$B$，它們的交集為空集，則前綴碼的一類編碼形態為：$B$, $AB$, $AAB$, $AAAB$, $AAAAB$，以此類推。如此，觀察到$B$集合的字符時，便可以確定當前漢字編碼的結束。
...
舉例：一個前綴碼方案中，「日」的編碼爲`ri`，則不存在任意一個字，它的前兩碼是`ri`。
...
松煙：純形碼。將字根的最後一碼設計為集合`aeiouv`中的字母，這樣當出現該集合字母時，就可以確定當前編碼的結束。
...
部分輸入法可以同時結合前綴和頂功兩種屬性。由於頂功的編碼分割是外向性的，在排除頂功前，無法確定當前編碼是當前漢字的編碼還是前序漢字的後續編碼。因此，頂功屬性高於前綴屬性，此類輸入法應歸為頂功碼。
...
逸碼：純形碼，頂功碼。前兩碼的編碼的字符集合為$A$，後續編碼的字符集合為$B$，且$A$和$B$的交集為空集。編碼模式包括：$AA$, $AAB$, $AABB$, $AABBB$ 等。因為任意數量編碼模式連綴非其他任意數量編碼模式連綴，故而具有頂功屬性。衹要出現$AA$模式，就可以確定前序漢字編碼的結束。

C42輸入法：純形碼，頂功碼。結合前綴碼和頂功碼屬性，編碼模式為：$AA$, $AABA$ 等。因為任意數量編碼模式連綴非其他任意數量編碼模式連綴，故而具有頂功屬性。衹要出現$BA$模式，就可以確定本漢字編碼的結束；衹要出現$AAAA$模式，就可以確定前序漢字編碼的結束。

[徐碼五二頂](https://github.com/Ace-Who/rime-xuma)：純形碼，頂功碼。結合前綴碼和頂功碼屬性，編碼模式為：$AA$, $AAAB$, $AAAAB$ 等。因為任意數量編碼模式連綴非其他任意數量編碼模式連綴，故而具有頂功屬性。衹要出現$B$，就可以確定本漢字編碼的結束；衹要出現$AAAAA$模式，就可以確定前序漢字編碼的結束。
...
字根大碼音聯，又稱「大碼音托」，是指在輸入法中，部分字根通過其**讀音**來聯繫起來，並賦予它們相同的大碼。
...
字根大碼形聯，又稱「大碼形托」，是指在輸入法中，部分字根通過其**形態**來聯繫起來，並賦予它們相同的大碼。
