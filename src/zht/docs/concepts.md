...
一般地，一個漢字會被分解為若干字根或筆劃，每一個字根或筆劃對應一個或多個編碼字符。
...
比如，某輸入法中，「甜」字可以依據字形拆分為「舌」「甘」兩個字根，以及左右結構。假設「舌」的編碼是`she`、「甘」的編碼是`gan`、左右結構的編碼是`zuoyou`，則「甜」字的編碼為`sheganzuoyou`。因為此編碼不依賴於「甜」字的讀音，此輸入法仍然是**純形**的。
...
例如，某輸入法中的所有漢字編碼的最後一碼都必然是`a`，且`a`只作為末尾編碼，則它具有自定屬性。當出現`a`時，就可以確定當前漢字編碼的結束。不需要看到後續漢字的編碼。
...
日月輸入法：純形碼。將字根的最後一碼設計為字根元音在五個元音字母`aeiou`上的映射，這樣當出現元音字母時，就可以確定當前編碼的結束。

靈明輸入法：純形碼。將字根的最後一碼設計為字根元音在五個元音字母`aeiou`上的映射，這樣當出現元音字母時，就可以確定當前編碼的結束。
...
最大長度為 N 的限長方案稱為「N碼限長」方案，簡稱「N限」方案。
...
在中文輸入法中，最大長度為 N 的限長方案往往被稱為「N碼定長」方案，簡稱「N定」方案。

:::

::: tip 方案舉例

五筆字形：全碼最短編碼長度為3，最長編碼長度爲4。它是四碼限長方案。  
倉頡：全碼最短編碼長度爲1，最長編碼長度爲5。它是五碼限長方案。
宇浩日月：全碼最短編碼長度為2，最長編碼長度為5。前綴碼，兼具免空格自分割特徵和限長特徵。它是五碼限長方案。

:::

#### 定長

狹義上的定長，指的是每個漢字的編碼長度固定。也就是説，無論輸入什麼漢字，編碼的長度都是一樣的，又稱為[絕對定長](#絕對定長)。
...
字根聚類，指的是將若干個字根按照某種内在特徵劃分為一組，並賦予相同的主要編碼（第一編碼、大碼）。
...
任何限長方案，在未達到其最大編碼長度時，在後方添加標點符號使候選漢字和標點同時上屏，同時具有頂功上屏和唯一上屏的特徵。這是因為漢字本身未達到唯一分割條件，看到後續字符（標點符號）後，纔確定了唯一分割條件，此時，漢字的上屏就是一種頂功上屏。而標點符號本身是自分割的，所以標點符號的上屏是唯一候選上屏。
...
詞語動態選重率，又稱「詞重」「詞語重碼」「詞頻加權選重率」「詞語動態重碼率」「理論詞語選重率」，是指在某一詞語集合内，編碼完全相同、且不為首選的詞語的**理論**出現頻率。它可以表達為：
...
多字詞動態選重率，有時也簡稱為「詞重」「詞語重碼」「理論詞語選重率」，可以完整稱為「詞語條件概率加權理論選重率」。它是指在某一詞語集合内，編碼完全相同、且不為首選的多字詞語的**理論**出現頻率。它體現為詞語動態選重率的條件概率。可以表達為：
