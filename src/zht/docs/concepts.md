...
「頂功」的「頂」字可以表現出它的外向性，通過匹配後續編碼的模式確定前序漢字編碼的結束，從而將前續漢字「頂」上屏幕。同時，「頂」字也表達出了**延遲上屏（頂字上屏）**的屬性。
...
宇浩輸入法系列：「<ruby>𠤎<rt>huá</rt></ruby><ruby>匕<rt>bǐ</rt></ruby>」二字音義不同，但字形非常接近，實際難以區分。因此被歸併到「匕」字根上，賦予相同的編碼。
