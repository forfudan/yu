...
逸碼：純形碼，頂功碼。前兩碼的編碼的字符集合爲$A$，後續編碼的字符集合爲$B$，且$A$和$B$的交集爲空集。編碼模式包括：$AA$, $AB$, $AAB$, $ABB$, $AABB$, $ABBB$, $AABBB$, $ABBBB$ 等。因爲任意數量編碼模式連綴非其他任意數量編碼模式連綴，故而具有頂功屬性。衹要出現$BA$模式，就可以確定前序漢字編碼的結束。
