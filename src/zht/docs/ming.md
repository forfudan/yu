...
本方案基於宇浩拆分，爲 25.5 鍵、編碼自分割、五碼、三編、音托、全字集、繁簡通打輸入方案。本方案開創性極强，同傳統限長方案輸入體驗有所不同，請務必充分瞭解、分析、平衡其風險和收益後再決定是否學習使用。

:::

## 簡介

**日月輸入法**，又稱**大明輸入法**、**明碼**，基於**宇浩拆分**設計製作，是世上第一款**編碼自分割**^[又稱前綴碼、前綴自由碼、免前綴碼、free-prefix code。]純形輸入方案，可完全離開空格鍵進行輸入。其名源自《尚書大傳》之「日月有常，星辰有行」。
...
本方案具有編碼自分割屬性，因此簡碼效率顯著高於一般的四碼限長方案，簡碼設置的**邊際效率**相對更高。綜上所述，在記憶500個簡碼的情況下，單字平均碼長約爲3。
...
本方案爲首款編碼自分割純形輸入方案，優缺點都十分突出，故總結如下：
...
這個方案，雖然在按鍵區分了A區和B區，但它不同於一般意義上的**頂功**方案。頂功方案中，只有輸入了下一個字的A區鍵，纔意味著上一個字輸入完畢，可以上屏，它對編碼的分割是外生的。而本方案中，只要出現B區按鍵（韻碼），便意味着這個字已經輸入完畢，可以上屏了，它對編碼的分割是內生的。在這個方案中，A區按鍵，如同珠子一樣串聯在B區按鍵前。任何字的編碼，都不是其他字編碼的前幾碼。我稱之為「編碼自分割」方案。在編碼學上，它被稱作**free-prefix code**（前綴自由碼）。
