...
│ 大碼 ┆ 字根                                                                                                                          │
╞══════╪═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╡
│ b    ┆ 虎hu 卜bu 魚e 灬bi 馬ma 爪sa 瓜ga 亦i 示ki                                                                                    │
...
│ h    ┆ 辰re 其qi 舟se 忄xo 自zi 亥he 下xa 丁di 乌u 鸟ni 冊ce 习xi 勹bi 冂ji 走ze 齒ri 止si 田ti 页e 贝bo                             │
│ j    ┆ 刀di 门me 丰fe 見ja 夕xi 矢ki 心xo 上ke 寸ci 巳si 巴ba 早zi 曰e 日i 片pa 爿pa 鬼go                                            │
...
│ m    ┆ 艮ge 耳o 不bu 丑re 七qi 乜me 也e 工gi 瓦a 艹ci 卅sa 虫ri 風fe                                                                 │
...
│ v    ┆ 见ja 山ka 业e 米mi 讠a 车re 乂i 士ki 土du 斤jo 戶hu                                                                           │
...
- 兩根字：首字首根大碼 + 首字首根聲碼 + 首字次根大碼 + 次字編碼，截取前五碼。如：
  - 「宇浩」的編碼是`DMF + CK`。
  - 「一下」的編碼是`F + HXA`。
  - 「一些」的編碼是`F + HSLS`。
- 三根字：首字首根大碼 + 首字首根聲碼 + 次字首根大碼 + 次字首根聲碼 + 第三字編碼，截取前五碼。
  - 「好不好」的編碼是`CN + MB + C`。
  - 「不知道」的編碼是`MB + JK + X`。
- 四根字：首字首根大碼 + 次字首根大碼 + 第三字首根大碼，第四字編碼，截取前五碼。
  - 「不置可否」的編碼是`M + G + H + MB`。
  - 「自作自受」的編碼是`H + W + H + BS`。
- 五根字及以上：首字首根大碼 + 次字首根大碼 + 第三字首根大碼 + 第四字首根大碼 + 末字首根大碼。
