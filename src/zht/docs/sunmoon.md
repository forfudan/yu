...
::: warning 注意
本方案基於宇浩拆分，爲 25.5 鍵、近亂序、音托、前綴碼、繁簡通打方案。初始學習難度較大，請務必充分瞭解、分析、平衡其風險和收益後再決定是否學習使用。
:::
...
1. 大碼在 `BCDFGHJKLMNPQRSTVWXY` 鍵上近亂序分佈（保留部分程度的聚類），共 20 鍵。
...
每個詞最多五碼，前四碼同星陳方案。第五碼爲日月特有，取**末字末根編碼**。
...
- 四字詞：首字第一碼 + 次字第一碼 + 第三字第一碼 + 第四字第一碼 + 第五碼。
  ...
  - 「恍恍惚惚」的編碼是`J + J + J + J + J`。
  - 「魑魅魍魉」的編碼是`J + J + J + J + W`。
- 五字及以上詞：首字第一碼 + 次字第一碼 + 第三字第一碼 + 第四字第一碼 + 第五碼（末字第一碼）。
...
日月方案的單字平均編碼（簡體文本）長度如下：

1. 單字全碼：4.32
1. 使用5個一碼上屏字：3.96
1. 額外使用73個二碼上屏字：3.53
1. 額外使用300個三碼上屏字：3.14（節約平均碼長超過0.0002的字設置簡碼）
1. 額外使用112個三碼上屏字：3.13（節約平均碼長超過0.0001的字設置簡碼）

也就是説，在記憶378個簡碼的情況下，單字平均碼長爲3.14。
由於本方案是前綴碼，因此簡碼效率顯著高於一般的四碼定長方案，簡碼設置的**邊際效率**相對更高。
...
- 單字平均碼長在78個簡碼情況下爲3.53，在625個簡碼情況下爲3.22。不適合追求极致碼長的用户。
- 更專注於單字。
- 簡碼不是全碼的前綴。
