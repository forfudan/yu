...
1. 使用5個一碼上屏字：3.96
1. 額外使用73個二碼上屏字：3.53
1. 額外使用482個三碼上屏字：2.99（節約平均碼長超過0.0002的字設置簡碼）

繼續填滿三碼空餘簡碼位對於碼長的降低不再顯著，考慮到邊際學習成本遞增，不再為低頻字設置簡碼。

本方案是前綴碼，因此簡碼效率顯著高於一般的四碼定長方案，簡碼設置的**邊際效率**相對更高。綜上所述，在記憶560個簡碼的情況下，單字平均碼長約爲3。
...
下表爲主要的二碼上屏簡碼字詞（空格表示此處是字根字），追求極致碼長的用戶必須熟悉。
...
下表爲主要的三碼上屏簡碼字（空格表示此處是簡碼詞），追求極致碼長的用戶可考慮熟悉。
