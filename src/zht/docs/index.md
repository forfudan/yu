...
const question = {
  title: '你是否想完全脫離空格鍵進行精確輸入？',
  description: '一般來説，傳統的四碼限長輸入法都需要按空格鍵上屏，特別是單字簡碼派，空格鍵使用率可以高達 30%。這對於大拇指可能是一種負擔。\n宇浩首創的無空格自分割模式，可以讓你完全不按空格鍵也能精確輸入漢字。這是二零二五年漢字輸入法領域最大的突破。\n當然，無空格的代價是更高的字根編碼學習成本，尤其對於第一次接觸字形輸入法的用户來説更是如此。',
  yesText: '我傾向於少按或不按空格鍵',
  noText: '多按些空格鍵沒有問題',
  // 選"是"後顯示的第二題
  yesNext: {
    title: '你是否接受更高的全碼碼長以換取更好的性能？',
    description: '大多數的字形輸入法的全碼是最長四碼。它的好處是在碼長和性能之間找到了一個良好的平衡點，且四碼的按鍵節奏感更佳。\n無空格自分割模式下，可以通過將全碼增加到五碼來換取更低的選重率、更好的手感、以及更簡單的規則，再通過記憶簡碼來減少碼長到較低水平。\n當然，這樣做的代價是，如果你不願意打簡碼，那麼全碼的碼長會更長。',
    yesText: '我願意接受更高的全碼碼長',
    noText: '我希望全碼碼長保持在四碼以內'
  },
  // 選"否"後顯示的第二題
  noNext: {
    title: '你是否接受完全亂序的字根佈局以換取更好的性能？',
    description: '大多數的字形輸入法，字根的排佈是遵照一定規則的，比如將相似的字根放在一個按鍵上，又比如根據字根的第一筆決定按鍵等。這樣做的好處是學習成本較低，也不容易遺忘。\n我們也可以通過將字根完全亂序分佈在鍵盤按鍵上，來換取更低的選重率、更好的手感、以及更簡單的規則。\n當然，這樣做的代價是，學習成本會更高，維護成本也會更高，一段時間低強度使用會導致遺忘。',
    yesText: '我願意接受亂序字根佈局',
    noText: '我希望字根的佈局有規律可循'
  }
}
...
const results = {
  'YY': {
    name: '日月',
    description: '適合追求極致的專業漢字工作者和輸入法發燒友',
    features: [
      '最長五碼，無空格輸入，形碼新潮流',
      '大碼半聚類半亂序分佈',
      '編碼規則簡單直接',
  ...
  'YN': {
    name: '靈明',
    description: '碼長、性能、規則、新潮的平衡之選',
    features: [
      '最長四碼，無空格輸入，形碼新潮流',
      '50個簡碼下的碼長即突破傳統四定方案極限',
      '大碼更加聚類分佈',
      '小碼基本不用特別學習',
      '需要特別記憶大小兩類字根'
    ],
  ...
  'NN': {
    name: '星陳',
    description: '形碼新手的可靠夥伴，難度性能的經典平衡',
    features: [
      '大碼高度聚類分佈，學習成本低',
      '選重率和手感良好',
      '編碼規則較爲簡單'
...
因爲編碼資源的有限性，每個輸入法都有自己的設計目標和取捨。有優點的同時也會有缺點。大家需要根據自己的需求和偏好，選擇其中最適合自己的那個輸入方案。下面的兩個問題，或許能幫助你做出選擇：

<MethodSelector 
  :question="question"
  :results="results"
  selectorTitle="幫你挑選輸入方案"
  resultTitle="推薦結果"
  resultMethodTemplate="你適合 {name} 輸入法"
  resetButtonText="重新測試"
  learnMoreText="詳細瞭解"
/>

## 系列輸入法的特點

宇浩系列中的每一個輸入法，都是[純字形的漢字輸入方案](./concepts#字形輸入法)：通過輸入漢字的部首和筆畫來輸入漢字，而不依賴漢字的具體讀音，也就是所謂「會寫就會打」「看到即輸入」。

這是如何做到的呢？答案是三個步驟：

- 第一步，也是宇浩輸入法的核心：將每一個漢字拆成不超過四個[字根](./concepts#字根)。
...
- 字頻加權後的[選重率](./concepts#選重)極低，基本上一字一碼，碼到字出，極其適合盲打。
- [繁簡字碼位分離](./concepts#繁簡通打)，比如`简` `簡`不同編碼，繁體簡體混在一起打也極少選重。
- 字形兼容大陸通規、[大陸古籍通規](./guji)[^guji_standards]、臺灣正體、香港繁體四套標準，方便檢字。
- 設有簡體、繁體兩套詞庫，字形涵蓋兩岸三地三套標準 [^opencc]。
- 全面覆蓋 [CJK 全字集](./concepts#中日韓統一漢字cjk) 101984 個漢字（包括至擴展 J 區），生僻字一網打盡。
- 檢字方便，翻頁少。總共 101984 個漢字，任何編碼候選項不超過 24 個。
- 採用大字根，拆字直觀，不會將漢字拆得零碎。
- 拆字規範，規則優先級明確，兼顧「邏輯性」和「直觀性」。追求一字一拆、無歧義。
- 兼顧手感。[速度當量](./concepts#速度當量)、互擊率領先。
...
評價一款輸入法，不能只看重碼率，因爲每一款輸入法都有自己的**設計哲學**和**目標用户**。有優點就必然有缺點，反之亦然。評價一款輸入法的維度，除卻重碼率，還有規則簡易度、字根複雜度、按鍵舒適度、平臺通用性等。這就是爲什麽拼音重碼高，但大多數人還是會使用拼音，因爲它的學習成本基本爲零。選擇學習輸入法，一定要符合自己的需求，要綜合考慮多方面因素，例如，是否願意背較多的字根，是否有打古文、繁體字需求等。
