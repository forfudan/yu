...
1. 使用5个一码上屏字：3.96
1. 额外使用73个二码上屏字：3.53
1. 额外使用482个三码上屏字：2.99（节约平均码长超过0.0002的字设置简码）

继续填满三码空余简码位对于码长的降低不再显著，考虑到边际学习成本递增，不再为低频字设置简码。

本方案是前缀码，因此简码效率显著高于一般的四码定长方案，简码设置的**边际效率**相对更高。综上所述，在记忆560个简码的情况下，单字平均码长约为3。
...
下表为主要的二码上屏简码字词（空格表示此处是字根字），追求极致码长的用户必须熟悉。
...
下表为主要的三码上屏简码字（空格表示此处是简码词），追求极致码长的用户可考虑熟悉。
